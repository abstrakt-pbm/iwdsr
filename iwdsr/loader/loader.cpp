...
void Loader::loadElf(ELF_PARSER::ELF& elfFile) {
    ...
    memMap.makeNamedMapping( "executable", elfMemBlk);
    if ( symbolResolver.isImageNeedToBeResolved( "executable" ) ) {
...
void Loader::loadDLL( std::string libName, ELF_PARSER::ELF& dynamicLib) {
    ...
    memMap.makeNamedMapping(libName, dllMemBlk);
    if ( symbolResolver.isImageNeedToBeResolved( libName ) ) {
